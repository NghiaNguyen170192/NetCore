...
2. **Upgrade Identity stack to net10.0** — Retarget NetCore.IdentityProvider.csproj and NetCore.Infrastructure.AuthenticationDatabase.csproj from net6.0 to net10.0, updating IdentityServer4 dependencies.
...
4. **Create PostgreSQL migrations** — Generate EF Core migrations for both ApplicationDatabaseContext.cs and ApplicationDbContext.cs targeting PostgreSQL, create migration service logic in NetCore.Migration.
...
6. **Consolidate Aspire orchestration** — Merge existing Aspire AppHost (src/client/NetCore/AppHost/Program.cs) with docker-compose services (PostgreSQL, Redis, migration, API, UI), remove duplicate docker-compose files, ensure single source of truth.
...
3. **Aspire AppHost structure** — Should the unified Aspire host (currently in `src/client/NetCore/AppHost/`) move to solution root for clarity, or remain in client folder? How to handle PostgreSQL + Redis resource definitions and health checks?
