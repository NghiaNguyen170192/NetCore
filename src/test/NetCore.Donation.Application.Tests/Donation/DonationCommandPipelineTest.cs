...
{
    [TestMethod]
    public async Task UserMakesDonation_PersistsContactMethodAndScheduleWithoutTransaction()
    {
        ...
        Assert.AreNotEqual(Guid.Empty, result.PaymentScheduleId);
        Assert.IsFalse(result.IsRecurring);
        Assert.AreEqual(RecurringInterval.OneOff, (await context.PaymentSchedules.SingleAsync()).RecurringInterval);
        ...
        Assert.IsTrue(messageTypes.Any(type => type.Contains(nameof(DonationCreatedDomainEvent))));
    }
    ...
    public async Task OutboxPipeline_WhenTransactionSucceeds_CreatesReceiptAndJournalInParallel()
    {
        ...
        Assert.AreEqual(result.PaymentScheduleId, transaction.PaymentScheduleId);
